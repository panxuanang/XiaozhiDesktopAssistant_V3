...
# High-level tools are always the preferred surface. This keeps MCP schemas small
# and lets the Windows client decide locally whether an API call is actually needed.
mcp.tool(description="打工人搭子统一入口。本地能完成的任务优先本地完成；只有写作、语义改写或模糊规划才使用客户端配置的 AI API。适合：看老板微信、处理附件、Excel、找文件、网页总结、定时交差。")(
    workmate.workmate
)
...
mcp.tool(description="本地提取式总结当前浏览器网页，不调用 AI API。") (workmate.local_summarize_current_webpage)
mcp.tool(description="本地提取式总结 PDF，不调用 AI API。") (workmate.local_summarize_pdf)
...
# Keep the general API-driven agent as a fallback for tasks outside the deterministic
# local-first router. The Xiaozhi prompt should prefer workmate first.
mcp.tool(description="通用电脑智能体兜底入口，会使用客户端配置的 AI API 自主选择工具。只有 workmate 无法确定性完成时再使用。")(
    desktop_agent
)
...
# Optional compatibility/developer mode: expose all low-level tools. Off by default
# to reduce tools/list payload and improve routing reliability on Xiaozhi.
if settings.modules.expose_low_level_tools:
    ...
    if _enabled("browser"):
        mcp.tool(description="打开网页。") (browser.browser_open)
        mcp.tool(description="搜索网页。") (browser.browser_search)
        mcp.tool(description="读取当前网页 DOM 正文。") (browser.browser_current_page)
        mcp.tool(description="用 AI 总结当前网页。") (browser.summarize_current_webpage)
